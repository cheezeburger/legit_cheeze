...
class MacroController:
    ...
    def loop(self):
        ...
        if not self.pet_feed_time or time.time() - self.pet_feed_time > 53:
            ...
            self.player_manager.castSkill('o', 0.2, sleep_first=True)
        if not self.abyssal_time or time.time() - self.abyssal_time > 200:
            print('Abyssal Lightning')
            self.abyssal_time = time.time()
            self.player_manager.castSkill('comma', 0.2, sleep_first=True)
        ...
        if not self.altar_time or time.time() - self.altar_time > 45 and \
            ...
            self.player_manager.spawn_altar()
        # =========Attack Section=========
        ...
        if self.attack_direction == 'left':
            ...
            if self.current_platform_hash == self.bottom_plat:
                # self.attack_left()
                self.player_manager.a_telecast()
