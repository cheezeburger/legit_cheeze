...
def macro_loop(input_queue, output_queue):
    ...
    try:
        ...
        output_queue.put(["log", "!! An error occurred in the bot process. Please check the log file. !!", ])
...
class MainScreen(tk.Frame):
    def __init__(self, master, user_id="null", expiration_time=0):
        ...
        self.menubar.add_command(label="Terrain Settings", command=lambda: PlatformDataCaptureWindow())
        self.menubar.add_command(label="Keyboard Settings", command=lambda: SetKeyMap(self.master))
        ...
        self.macro_pid_infotext.set("Not running")
        ...
        tk.Label(self.macro_info_frame, text="Bot status:").grid(row=0, column=0)
        ...
        self.macro_process_toggle_button = tk.Button(self.macro_info_frame, text="Running",
                                                     command=self.toggle_macro_process)
        ...
        tk.Label(self.macro_info_frame, textvariable=self.platform_file_name).grid(row=1, column=1,
                                                                                   sticky=N + S + E + W)
        self.platform_file_button = tk.Button(self.macro_info_frame, text="Select a file",
                                              command=self.onPlatformFileSelect)
        self.platform_file_button.grid(row=1, column=2, sticky=N + S + E + W)

        self.macro_start_button = tk.Button(self.macro_info_frame, text="Bot start", fg="green",
                                            command=self.start_macro)
        self.macro_start_button.grid(row=2, column=0, sticky=N + S + E + W)
        self.macro_end_button = tk.Button(self.macro_info_frame, text="Bot shutdown", fg="red", command=self.stop_macro,
                                          state=DISABLED)
        ...
        # self.log(
        #     "MS-Visionify는 상업적 목표로 제작된 프로그램이 아니기에 무료이며, https://github.com/Dashadower/MS-Visionify 에 모든 소스가 공개되어 있습니다.")
        # self.log(
        #     "Please be known that using this bot may get your account banned. By using this software, you acknowledge that the developers are not liable for any damages caused to you or your account.")
        # self.log(
        #     "MS-Visionify was not created for commercial purposes and thus free of charge. The entire source code can be found at https://github.com/Dashadower/MS-Visionify")
    ...
    def check_input_queue(self):
        while not self.macro_process_in_queue.empty():
            ...
            if output[0] == "log":
                ...
                self.log("Bot process terminated.")
                ...
                self.log("The bot process terminated due to an error. Please check the log file.")
    ...
    def start_macro(self):
        ...
        if not keymap:
            showerror(APP_TITLE, "Failed to read key settings. Please reset the key.")
        else:
            if not self.platform_file_dir.get():
                ...
                if not MapleScreenCapturer().ms_get_screen_hwnd():
                    showwarning(APP_TITLE, "Maple window was not found. Please run Maple")
                else:
    ...
    def stop_macro(self):
        self.macro_process_out_queue.put(("stop",))
        self.log("Complete bot stop request. Please wait a moment until it stops.")
        self.macro_end_button.configure(state=DISABLED)
        self.macro_start_button.configure(state=NORMAL)
        self.platform_file_button.configure(state=NORMAL)
    ...
    # GUI modifier when macro process starts
    def toggle_macro_process(self):
        if not self.macro_process:
            ...
            # Starts executing macro_loop here
            p = multiprocessing.Process(target=macro_loop,
                                        args=(self.macro_process_out_queue, self.macro_process_in_queue))
            p.daemon = True
    ...
    # Called when user clicks select terrain
            ...
            if os.path.exists(platform_file_path):
                with open(platform_file_path, "rb") as f:
                    try:
                        ...
                        self.log("Number of platforms: ", len(platforms.keys()), "Number of one-way platforms:",
                                 len(oneway_platforms.keys()))
                    except:
                        showerror(APP_TITLE,
                                  "File verification error\n File: %s\nFile verification failed. Please check if the file is broken." % (
                                      platform_file_path))
                    else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="LegitCheeze, a legit MSEA bot.")
...
# from macro_script_astar_test import MacroControllerAStar as MacroController
#
